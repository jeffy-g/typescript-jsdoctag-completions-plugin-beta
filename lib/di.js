...
function createPresetEntries(nameData, translations, syntaxMap, synonymsMap, isBlock) {
    ...
    for (let index = 0, end = nameData.length; index < end;) {
        ...
        /** @type {Writable<JSDocTagPresetAPI.TJSDocTagPresetEntry>} */
 ...
 * @date 2020/9/5
 */
const compose = (rawPreset, translations, syntaxes) => {
    return () => {
        return injectDependencies(rawPreset, translations, syntaxes);
    };
};
